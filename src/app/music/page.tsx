...
export default function MusicPage() {
  ...
  const [currentSource, setCurrentSource] = useState<'netease' | 'qq' | 'kuwo'>(
    'netease',
  );
  ...
  const loadUserPlaylistSongs = async (playlistId: string) => {
    try {
      ...
      const response = await fetch(
        `/api/music/playlists/songs?playlistId=${playlistId}`,
      );
      if (response.ok) {
        const data = await response.json();
  ...
  const handleDeleteUserPlaylist = async (playlistId: string) => {
    setConfirmModal({
      ...
      onConfirm: async () => {
        ...
        try {
          const response = await fetch(
            `/api/music/playlists?playlistId=${playlistId}`,
            {
              method: 'DELETE',
            },
          );
    ...
    if (audioRef.current) {
      if (isPlaying) {
        ...
        if (currentSong && playlistIndex >= 0 && playRecords[playlistIndex]) {
          ...
          saveMusicPlayRecord(record.platform, record.id, dbRecord).catch(
            (err) => {
            ...
            <div className='flex items-center w-full md:flex-1 md:max-w-md md:ml-auto h-10 md:h-9 gap-2'>
              ...
              <div className='relative group w-full h-full'>
                <div className='absolute inset-y-0 left-3 flex items-center pointer-events-none'>
                  <svg
                    className='w-4 h-4 text-zinc-500 group-focus-within:text-white'
                    fill='none'
                    stroke='currentColor'
                    viewBox='0 0 24 24'
                  >
            ...
            {currentView === 'playlists' && !loading && (
              <div>
                <div className='flex items-center justify-between mb-6 border-b border-white/5 pb-2'>
                  <h2 className='text-xs font-mono text-white/50 tracking-widest'>
                    排行榜
                  </h2>
                  ...
                  {songs.map((song, index) => (
                    ...
                    >
                      ...
                      >
                        ...
                        <div className='text-xs text-zinc-500 truncate md:hidden'>
                          {song.artist}
                        </div>
                      </div>
                      <div
                      ...
                      {loadingUserPlaylistSongs ? (
                        ...
                        <div className='space-y-2'>
                          {userPlaylistSongs.map((song, index) => (
                            ...
                            >
                              <div className='text-zinc-500 text-sm w-8 text-center'>
                                {index + 1}
                              </div>
                              {song.pic && (
                                <img
            ...
            >
              ...
              <div className='relative h-32 md:h-48 bg-gradient-to-b from-zinc-800 to-zinc-900 shrink-0'>
                ...
                <div className='relative h-full flex flex-col items-center justify-center p-4 md:p-6'>
                  ...
                  <h2 className='text-base md:text-xl font-bold text-white text-center mb-1 line-clamp-1'>
                    {currentSong.name}
                  </h2>
                  ...
                  >
                    ...
                    {playMode === 'single' && (
                      <div className='relative w-4 h-4 md:w-5 md:h-5'>
                        ...
                        <span className='absolute inset-0 flex items-center justify-center text-[7px] md:text-[8px] font-bold'>
                          1
                        </span>
                      </div>
                    )}
                    ...
                    {playlist.map((song, index) => (
                      ...
                      >
                        ...
                        >
                          ...
                          {index === playlistIndex ? (
                            <svg
                              className='w-5 h-5 text-green-400 shrink-0 animate-pulse'
                              fill='currentColor'
                              viewBox='0 0 20 20'
                            >
                              <path d='M18 3a1 1 0 00-1.196-.98l-10 2A1 1 0 006 5v9.114A4.369 4.369 0 005 14c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V7.82l8-1.6v5.894A4.37 4.37 0 0015 12c-1.657 0-3 .895-3 2s1.343 2 3 2 3-.895 3-2V3z' />
                            ...
                            <svg
                              className='w-5 h-5 text-zinc-600 group-hover:text-white transition-colors shrink-0'
                              fill='currentColor'
                              viewBox='0 0 20 20'
                            >
                              <path d='M6.3 2.841A1.5 1.5 0 004 4.11V15.89a1.5 1.5 0 002.3 1.269l9.344-5.89a1.5 1.5 0 000-2.538L6.3 2.84z' />
            ...
            >
              ...
              <div className='p-4 space-y-2'>
                ...
                >
                  <div className='flex items-center gap-3'>
                    <div
                      className={`w-2 h-2 rounded-full ${quality === '128k' ? 'bg-amber-400' : 'bg-zinc-600'}`}
                    />
                ...
                >
                  <div className='flex items-center gap-3'>
                    <div
                      className={`w-2 h-2 rounded-full ${quality === '320k' ? 'bg-amber-400' : 'bg-zinc-600'}`}
                    />
                    <div className='text-left'>
                ...
                >
                  <div className='flex items-center gap-3'>
                    <div
                      className={`w-2 h-2 rounded-full ${quality === 'flac' ? 'bg-amber-400' : 'bg-zinc-600'}`}
                    />
                    <div className='text-left'>
                ...
                >
                  <div className='flex items-center gap-3'>
                    <div
                      className={`w-2 h-2 rounded-full ${quality === 'flac24bit' ? 'bg-amber-400' : 'bg-zinc-600'}`}
                    />
                    <div className='text-left'>
